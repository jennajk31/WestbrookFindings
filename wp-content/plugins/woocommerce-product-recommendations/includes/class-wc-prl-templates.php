...
class WC_PRL_Templates {
	...
	public function process_hook( $hook, $ajax = false, $args = array() ) {
		...
		if ( ! $ajax && wc_prl_render_using_ajax() ) {
			...
			printf( '<div class="wc-prl-ajax-placeholder" id="%s" data-env="%s"></div>', esc_attr( $hook ), esc_attr( json_encode( $environment ) ) );
	...
	public function render_deployment( $products ) {
		...
		if ( empty( $products ) && ! is_null( $products ) ) {
			...
			wc_get_template(
				...
				false,
				WC_PRL()->get_plugin_path() . '/templates/'
			);
	...
	public function frontend_scripts() {
		...
		wp_script_add_data( 'wc-prl-main', 'strategy', 'defer' );
		...
		$params = apply_filters(
			'woocommerce_prl_front_end_params',
			array(
