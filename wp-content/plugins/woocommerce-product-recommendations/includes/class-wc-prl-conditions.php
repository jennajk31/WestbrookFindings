...
class WC_PRL_Conditions {
	...
	public function instantiate_conditions() {
		...
		$load_conditions = apply_filters(
			'woocommerce_prl_conditions',
			array(
	...
	public function get_admin_conditions_html( $engine_type, $options = array() ) {
		...
		?>
		<div id="os-container" class="widefat wc-prl-conditions-container"
			data-os_count="<?php echo (int) $conditions_count; ?>"
			data-os_post_name="<?php echo esc_attr( $post_name ); ?>">
			<div class="os_boarding<?php echo $conditions_count ? '' : ' active'; ?>">
				<div class="icon">
					<i class="prl-icon prl-eye"></i>
				</div>
				<div
					class="text"><?php esc_html_e( 'Add conditions to control the visibility of these product recommendations.', 'woocommerce-product-recommendations' ); ?></div>
				<div
					class="text no_engine"><?php esc_html_e( 'To add visibility conditions, an Engine must be selected.', 'woocommerce-product-recommendations' ); ?></div>
			</div>
			<div id="os-list"<?php echo $conditions_count ? '' : ' class="hidden"'; ?>>
				<?php
							...
							if ( array_key_exists( $condition_id, $conditions ) ) {
								...
								<div class="os_row" data-os_index="<?php echo esc_attr( $condition_index ); ?>">
									...
									<div class="os_remove column-wc_actions">
										<a href="#" role="button"
											aria-label="<?php echo esc_attr__( 'Remove condition', 'woocommerce-product-recommendations' ); ?>"
	...
	private function get_conditions_dropdown( $engine_type, $conditions, $selected_id, $additional_options = array() ) {
		...
		<select class="os_type">
			...
			foreach ( $conditions as $condition_id => $condition ) {
				?>
				<option
					value="<?php echo esc_attr( $condition_id ); ?>" <?php echo $condition_id === $selected_id ? 'selected="selected"' : ''; ?>>
					<?php
		...
		foreach ( $types as $type => $label ) {
			...
			<script type="text/template" id="tmpl-wc_prl_engine_<?php echo esc_attr( $type ); ?>_condition_row">
				<div class="os_row" data-os_index="{{{ data.os_index }}}">
					...
					<div class="os_remove column-wc_actions">
						<a href="#" role="button"
							aria-label="<?php echo esc_attr__( 'Remove condition', 'woocommerce-product-recommendations' ); ?>"
							class="button wc-action-button trash help_tip"
			...
			foreach ( $conditions as $condition_id => $condition ) {
				...
				<script type="text/template"
						id="tmpl-wc_prl_engine_<?php echo esc_attr( $type ); ?>_condition_<?php echo esc_attr( $condition_id ); ?>_content">
					<?php
