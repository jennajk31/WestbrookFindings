...
class WC_PRL_Deployment {
	...
	public function get_products( $force = false ) {
		...
		$cached_products = $this->data->get_meta_cache_data( $this->get_cache_key() );
		if ( ! empty( $cached_products ) ) {
			...
			$is_expired             = time() > ( absint( $cached_products['created_at'] ) + $refresh_interval );
			$engine_cache_version   = $engine->get_cache_version();
			$products_cache_version = isset( $cached_products['cache_version'] ) ? $cached_products['cache_version'] : '';

			// Empty cache version means it was generated before cache versioning was introduced.
			$engine_changed = ! empty( $engine_cache_version ) && $engine_cache_version !== $products_cache_version;
			if ( $force || $is_expired || $engine_changed ) {
				$this->set_expired( true );

				$is_throttled = WC_PRL()->deployments->is_generator_throttled();
				if ( ! $is_throttled ) {
					$is_queued = WC_PRL()->deployments->is_deployment_in_queue( $this );
					if ( ! $is_queued ) {
				...
				if ( wc_prl_debug_enabled() ) {
					...
					if ( $is_throttled ) {
						echo '<!-- Throttled regeneration for deployment #' . esc_html( $this->get_id() ) . ' -->';
					} elseif ( $is_queued ) {
						echo '<!-- Already in queue for deployment #' . esc_html( $this->get_id() ) . ' -->';
					} else {
						echo '<!-- Scheduling regeneration for deployment #' . esc_html( $this->get_id() ) . ' -->';
					}
				}
			} elseif ( wc_prl_debug_enabled() ) {
			...
			if ( wc_prl_debug_enabled() ) {
				...
				if ( $is_throttled ) {
					echo '<!-- Throttled initial generation for deployment #' . esc_html( $this->get_id() ) . ' -->';
				} elseif ( $is_queued ) {
					echo '<!-- Queued initial generation for deployment #' . esc_html( $this->get_id() ) . ' -->';
				} else {
					echo '<!-- Scheduling initial generation for deployment #' . esc_html( $this->get_id() ) . ' -->';
				}
			}
