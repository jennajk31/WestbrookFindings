...
class WC_PRL_Filters {
	...
	public function get_admin_filters_html( $engine, $options = array() ) {
		...
		?>
		<div id="os-container" class="widefat wc-prl-filters-container"
			data-os_count="<?php echo esc_attr( $filters_count ); ?>">
			<div class="os_boarding<?php echo $filters_count ? '' : ' active'; ?>">
				<div class="icon">
					<i class="prl-icon prl-filter"></i>
				</div>
				<div
					class="text"><?php esc_html_e( 'No filters found. Add one now?', 'woocommerce-product-recommendations' ); ?></div>
			</div>
			<div id="os-list"<?php echo $filters_count ? '' : ' class="hidden"'; ?>>
				<?php
							...
							if ( array_key_exists( $filter_id, $filters ) ) {
								...
								<div class="os_row" data-os_index="<?php echo esc_attr( $filter_index ); ?>">
									...
									<div class="os_remove column-wc_actions">
										<a href="#" role="button"
											aria-label="<?php echo esc_attr__( 'Remove filter', 'woocommerce-product-recommendations' ); ?>"
	...
	private function get_filters_dropdown( $engine_type, $filters, $selected_id, $additional_options = array() ) {
		...
		<select class="os_type">
			...
			foreach ( $filters as $filter_id => $filter ) {
				?>
				<option
					value="<?php echo esc_attr( $filter_id ); ?>" <?php echo $filter_id === $selected_id ? 'selected="selected"' : ''; ?>>
					<?php
		...
		foreach ( $types as $type => $label ) {
			...
			foreach ( $filters as $filter_id => $filter ) {
				...
				<script type="text/template"
						id="tmpl-wc_prl_engine_<?php echo esc_attr( $type ); ?>_filter_<?php echo esc_attr( $filter_id ); ?>_content">
					<?php
	...
	/**
	 * Pass keys from the WooCommerce query builder to the WP_Query object
	 *
