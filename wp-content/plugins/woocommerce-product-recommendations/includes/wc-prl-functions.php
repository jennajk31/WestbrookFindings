...
function wc_prl_get_contextual_engine_types() {
	return (array) apply_filters(
		'woocommerce_prl_contextual_engine_types',
		array(
			'product',
			'archive',
			'order',
...
function wc_prl_delete_engine_transients( $post_id = 0 ) {
	...
	/**
	 * 'woocommerce_delete_engine_transients' action.
	 *
	 * Depreacated since 4.0.0
	 */
 ...
 *
 * @since 1.0.0
 * @since 4.1.0 Made the engine parameter optional.
 *
 * @param  WC_PRL_Engine $engine Engine instance (optional).
 * @return int
 */
function wc_prl_get_cache_regeneration_threshold( $engine = null ) {
	 ...
	 */
	$threshold = ! is_null( $engine ) ? apply_filters( 'woocommerce_prl_cache_regeneration_threshold', $threshold, $engine ) : $threshold;
 ...
 * @since 4.1.0
 *
 * @param string $cookie Optional. Cookie value.
 * @return int[]
 */
function wc_prl_get_recently_viewed_cookie_products( $cookie = null ) {

	if ( is_null( $cookie ) ) {
		$cookie = isset( $_COOKIE['wc_prl_recently_viewed'] ) ? sanitize_text_field( $_COOKIE['wc_prl_recently_viewed'] ) : '';
	}
	...
	$parts = (array) explode( ',', sanitize_text_field( wp_unslash( $cookie ) ) );
	if ( empty( $parts ) || ! is_array( $parts ) || empty( $parts[0] ) ) {
		return array();
	}

  // phpcs:ignore Squiz.Commenting.InlineComment.InvalidEndChar
	$viewed_products = wp_parse_id_list( array_filter( (array) explode( '|', $parts[0] ) ) );// nosemgrep: audit.php.lang.misc.array-filter-no-callback
	if ( empty( $viewed_products ) ) {
		return array();
	}

	return array_map( 'absint', $viewed_products );
}
...
function wc_prl_print_taxonomy_tree_options( $terms, $selected_ids = array(), $args = array() ) {
	...
	foreach ( $terms as $term ) {
		...
		// Revert sanitize_title that happens on slugs when running the get_term query.
		if ( 'slug' === $key ) {
			$term->$key = urldecode( $term->$key );
		}
