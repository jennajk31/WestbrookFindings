		...
		switch ( $step_index ) {
			...
			case 1:
				...
				if ( empty( $total_customers_bought_source ) || $total_customers_bought_source < $customers_count_min ) {
					if ( wc_prl_debug_enabled() ) {
						WC_PRL()->log( sprintf( '[Failed] Step "%d" for deployment #%d and product #%d failed with status `%s`.', $step_index, $deployment_id, $source_id, "Total customers purchased the source product: {$total_customers_bought_source}. Reference Count Threshold: {$customers_count_min}" ), 'info', $log_context );
					}
					return null;
				}
			...
			case 2:
				...
				if ( $total_customers_bought_source ) {
					...
					$arguments = array( $source_id, $time_period_in_weeks, $support_number );
					if ( 'join' === $foreground_query_type ) {
						...
						$sql = "SELECT `t1`.`product_id`, count( distinct( `t1`.`customer_id` ) ) AS `count`
								...
								WHERE `t1`.`product_id` <> %1\$d" .
								( $time_period_in_weeks > 0 ? ' AND `t1`.`date_created` >= DATE_SUB( NOW(), INTERVAL %2$d WEEK )' : '' ) .
								' AND `t2`.`product_id` = %1$d
						...
						$sql = "SELECT `t1`.`product_id`, count( distinct( `t1`.`customer_id` ) ) AS `count`
								...
								" AND `t1`.`customer_id` IN (
									...
									WHERE `t2`.`product_id` = %1\$d" .
									( $time_period_in_weeks > 0 ? ' AND `t2`.`date_created` >= DATE_SUB( NOW(), INTERVAL %2$d WEEK )' : '' ) .
								')
								GROUP BY `t1`.`product_id`
								HAVING count > %3$d
								ORDER BY `count` DESC
