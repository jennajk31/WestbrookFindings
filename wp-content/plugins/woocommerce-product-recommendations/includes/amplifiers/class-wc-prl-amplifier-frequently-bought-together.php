...
class WC_PRL_Amplifier_Frequently_Bought_Together extends WC_PRL_Amplifier {
	 ...
	 *
	 * @param  int                     $step_index  Current step to run.
	 * @param  WC_PRL_Deployment|array $source      The deployment object or an array of source data.
	 * @param  array                   $args        Includes a set of all previous steps return values.
	 * @return mixed
	 */
	public function run_step( $step_index, $source, $args = array() ) {

		// Ensure that WC_PRL_Deployment is not required. Instead a single product ID can be passed.
		if ( $source instanceof \WC_PRL_Deployment ) {

			$deployment    = $source;
			$deployment_id = $deployment->get_id();
			$source_data   = $deployment->get_source_data();
			$source_id     = (int) array_pop( $source_data );
			$log_context   = 'wc_prl_generator_tasks';

		} elseif ( is_array( $source ) ) {

			$deployment    = isset( $source['calibration_type'] ) ? $source['calibration_type'] : 'default';
			$deployment_id = 0;
			$source_id     = isset( $source['product_id'] ) ? (int) $source['product_id'] : 0;
			$log_context   = 'wc_prl_amplifier_cache_tasks';
		...
		switch ( $step_index ) {
			...
			case 1:
				...
				if ( wc_prl_debug_enabled() ) {
					...
					WC_PRL()->log( sprintf( '[Benchmarking] Step "%d" run for %.4f seconds.', $step_index, $timediff ), 'info', $log_context );
				}
				...
				if ( empty( $total_orders_containing_source ) || $total_orders_containing_source < $orders_count_min ) {
					if ( wc_prl_debug_enabled() ) {
						WC_PRL()->log( sprintf( '[Failed] Step "%d" for deployment #%d and product #%d failed with status `%s`.', $step_index, $deployment_id, $source_id, "Total orders containing the source product: {$total_orders_containing_source}. Reference Count Threshold: {$orders_count_min}" ), 'info', $log_context );
					}
					return null;
				}
			...
			case 2:
				...
				if ( $total_orders_containing_source ) {
					...
					$arguments = array( $source_id );
					if ( $time_period_in_weeks > 0 ) {
						$arguments[] = $time_period_in_weeks;
					}
					if ( 'join' === $foreground_query_type ) {

						$arguments[] = $source_id;
						$arguments[] = $support_number;
						...
						$sql = "SELECT `t1`.`product_id`, count( `t1`.`order_id` ) AS `count`
								...
								WHERE `t1`.`product_id` <> %d" .
								( $time_period_in_weeks > 0 ? ' AND `t1`.`date_created` >= DATE_SUB( NOW(), INTERVAL %d WEEK )' : '' ) .
								' AND `t2`.`product_id` = %d
						...
						$sql = "SELECT `t1`.`product_id`, count( `t1`.`order_id` ) AS `count`
								...
								" AND `t1`.`order_id` IN (
									...
									WHERE `t2`.`product_id` = %d" .
									( $time_period_in_weeks > 0 ? ' AND `t2`.`date_created` >= DATE_SUB( NOW(), INTERVAL %d WEEK )' : '' ) .
								')
								GROUP BY `t1`.`product_id`
								HAVING count > %d
								ORDER BY `count` DESC
					...
					$prepared_sql = $wpdb->prepare( $sql, $arguments ); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
				...
				if ( empty( $counts ) ) {
					if ( wc_prl_debug_enabled() ) {
						...
						WC_PRL()->log( sprintf( '[Failed] Step "%1$d" for deployment #%2$d and product #%3$d failed with status `%4$s`.', $step_index, $deployment_id, $source_id, $status ), 'info', $log_context );
					}
			...
			case 4:
				...
				// Has cached data?
				$cached_frequencies = WC_PRL()->db->frequencies->query(
					array(
					...
					if ( ! empty( $results ) ) {
						...
						foreach ( $results as $result ) {
							...
							try {
								...
								WC_PRL()->log( sprintf( 'Failed to cache frequency. Input data: %s', print_r( $freq_cache_args, true ) ), 'warning', $log_context );
							}
			...
			case 5:
				...
				if ( ! empty( $products_to_calc ) && 0 != $total_orders_containing_source ) {
					...
					if ( empty( $significant_products ) && wc_prl_debug_enabled() ) {
						...
						WC_PRL()->log( sprintf( '[Failed] Step "%d" for deployment #%d and product #%d failed with status `%s`.', $step_index, $deployment_id, $source_id, $status ), 'info', $log_context );
					}
