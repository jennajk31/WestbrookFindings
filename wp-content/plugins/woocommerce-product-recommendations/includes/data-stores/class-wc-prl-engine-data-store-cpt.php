...
class WC_PRL_Engine_Data_Store_CPT extends WC_Data_Store_WP {
	...
	*/
	...
	public function update( &$engine ) {
		...
		if ( array_intersect( array( 'filters_data', 'amplifiers_data' ), array_keys( $changes ) ) ) {
	...
	*/
	...
	protected function read_engine_data( &$engine ) {
		...
		$engine->set_props(
			array(
				...
				'cache_version'   => get_post_meta( $id, '_cache_version', true ),
			)
		);
	}
	...
	protected function update_terms( &$engine, $force = false ) {
		...
		if ( $force || array_key_exists( 'type', $changes ) ) {
			$current_deployments = WC_PRL()->db->deployment->query(
				array(
					'return'    => 'ids',
	...
	protected function clear_caches( &$engine ) {
		WC_Cache_Helper::invalidate_cache_group( 'engine_' . $engine->get_id() );
	}
	...
	public function search_engines( $term, $type = array(), $limit = null ) {
		...
		if ( ! empty( $post_types ) ) {
			$post_type_where = "engines.post_type IN ('" . implode( "','", $post_types ) . "') ";
		}
