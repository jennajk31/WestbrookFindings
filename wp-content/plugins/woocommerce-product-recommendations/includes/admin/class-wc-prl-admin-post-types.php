...
class WC_PRL_Admin_Post_Types {
	...
	public static function init() {
		...
		add_filter( 'screen_options_show_screen', array( __CLASS__, 'hide_screen_options' ), 10, 1 );
	...
	public static function post_submitbox_start() {
		...
		$deployments = WC_PRL()->db->deployment->query(
			array(
				'return'    => 'count',
				'engine_id' => $post->ID,
			)
		...
		$url   = add_query_arg(
			array(
				'post'   => absint( $post->ID ),
				'action' => 'prl_engine_clear_caches',
			),
			admin_url( 'post.php' )
		);
	...
	public static function handle_clear_caches_action( $engine_id ) {
		...
		$engine->invalidate_cache_version();
		$engine->save();

		WC_PRL_Admin_Notices::add_notice( 'Cache invalidated. New recommendations will be regenerated shortly after the first time they are requested.', 'success', true );
		WC_PRL_Admin_Notices::save_notices();
		wp_safe_redirect(
			esc_url_raw(
				add_query_arg(
					array(
						'post'   => absint( $engine->get_id() ),
						'action' => 'edit',
					),
					admin_url( 'post.php' )
				)
	...
	/**
	 * Hide screen options for specific PRL admin screens.
	 *
	 * @param  bool $show_screen Whether to show the screen options tab.
	 * @return bool Returns false to hide screen options, true to show them
	 */
	public static function hide_screen_options( $show_screen ) {

		$screen            = get_current_screen();
		$prl_admin_screens = array(
		...
		// Hide screen options (return false) only for PRL admin screens.
		if ( $screen && in_array( $screen->id, $prl_admin_screens, true ) ) {
			return false;
		}

		// Return the original value for all other screens.
		return $show_screen;
	}
