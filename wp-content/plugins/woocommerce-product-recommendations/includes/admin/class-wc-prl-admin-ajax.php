...
class WC_PRL_Admin_Ajax {
	...
	*/
	...
	/**
	 * Regenerates deployment products.
	 * This is used when the user wants to regenerate the products for a deployment.
	 *
	 * @return void
	 */
	...
	*/
	...
	public static function regenerate_engine_products() {
		...
		if ( $engine->get_id() === 0 ) {
			$errors[] = __( 'No engine found for regeneration.', 'woocommerce-product-recommendations' );
		} else {
	...
	public static function json_search_engines( $term = '', $type = array() ) {
		...
		foreach ( $engine_objects as $engine_object ) {
			...
			// Add to results.
			$engines[ $engine_object->get_id() ] = array(
				'text' => rawurldecode( $formatted_name ),
				'type' => $engine_object->get_type(),
			);
		}
