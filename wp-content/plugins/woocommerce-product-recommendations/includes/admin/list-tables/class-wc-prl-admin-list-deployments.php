...
class WC_PRL_Deployments_List_Table extends WP_List_Table {
	...
	public function column_title( $item ) {
		...
		$title          = $item['title'] ? esc_html( $item['title'] ) : esc_html__( '(no title)', 'woocommerce-product-recommendations' );
		...
		printf(
			...
			$title, // phpcs:ignore WordPress.XSS.EscapeOutput.OutputNotEscaped,WordPress.Security.EscapeOutput.OutputNotEscaped
	...
	public function prepare_items() {
		...
		$this->items = WC_PRL()->db->deployment->query(
			// phpcs:ignore
	...
	public function no_items() {
		...
		if ( 0 < absint( $engines_count->publish ) ) {
			...
			<div class="prl-engines-empty-state">
				...
				<a class="components-button is-primary" id="sw-button-primary" href="<?php echo esc_url( admin_url( 'post-new.php?post_type=prl_engine' ) ); ?>"><?php esc_html_e( 'Create engine', 'woocommerce-product-recommendations' ); ?></a>
			</div>
			<?php
		}
	}
