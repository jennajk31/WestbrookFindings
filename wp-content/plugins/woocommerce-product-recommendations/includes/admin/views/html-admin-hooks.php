...
<div class="wrap woocommerce prl-locations-wrap">
	...
	<?php require __DIR__ . '/partials/html-admin-locations-tabs.php'; ?>
	...
	<div class="wc-prl-deployments wc-metaboxes-wrapper">
		...
		<div class="toolbar">
			<span class="engines_count">
			<?php
				/* translators: %s deployed engines counter */
				$count = isset( $map[ $selected_hook ] ) ? count( $map[ $selected_hook ]['deployments'] ) : 0;
			if ( $count > 0 ) {
				printf( esc_html( _n( '%s engine deployed', '%s engines deployed', $count, 'woocommerce-product-recommendations' ) ), (int) $count );
			}
			?>
			</span>
