...
<div class="wrap woocommerce">
	...
	<?php if ( ! $deployment ) { ?>
		...
		<h1 class="wp-heading-inline"><?php printf( esc_html__( 'Deploy &quot;%s&quot;', 'woocommerce-product-recommendations' ), $engine->get_name() ? esc_html( $engine->get_name() ) : esc_html__( '(untitled)', 'woocommerce-product-recommendations' ) ); ?></h1>
			...
			<?php } ?>
				...
				<?php if ( empty( $GLOBALS['hide_save_button'] ) ) : ?>
					<div class="sw-form-submit">
						<?php if ( ! $deployment ) { ?>
							<button name="save" class="components-button is-primary" id="sw-button-primary" type="submit" value="<?php esc_attr_e( 'Deploy', 'woocommerce-product-recommendations' ); ?>"><?php esc_html_e( 'Deploy', 'woocommerce-product-recommendations' ); ?></button>
							<a href="<?php echo ! $is_quick ? esc_url( admin_url( sprintf( 'post.php?post=%d&action=edit', $engine_id ) ) ) : esc_url( admin_url( 'admin.php?page=prl_locations' ) ); ?>" class="components-button is-tertiary"><?php esc_html_e( 'Cancel', 'woocommerce-product-recommendations' ); ?></a>
						<?php } else { ?>
							<input type="hidden" name="prl_deploy[id]" value="<?php echo esc_attr( $deployment->get_id() ); ?>">
							<button name="save" class="components-button is-primary" id="sw-button-primary" type="submit" value="<?php esc_attr_e( 'Save changes', 'woocommerce-product-recommendations' ); ?>"><?php esc_html_e( 'Save changes', 'woocommerce-product-recommendations' ); ?></button>
						<?php } ?>
					</div>
				<?php endif; ?>
