...
class WC_PRL_Deployment_DB {
	...
	public function query( $args ) {
		...
		} elseif ( in_array( $args['return'], array( 'ids' ), true ) ) {
		...
		$cache_key  = WC_Cache_Helper::get_cache_prefix( 'wc_prl_deployment_query' ) . $query_key . $params_key;
		$data       = wp_cache_get( $cache_key, 'wc_prl_deployment_query', false, $found );
		if ( true === $found ) {
			return $data;
		}
		...
		if ( 'count' === $args['return'] ) {
			...
			wp_cache_set( $cache_key, $count, 'wc_prl_deployment_query', DAY_IN_SECONDS );
		...
		// Store to runtime cache.
		wp_cache_set( $cache_key, $a, 'wc_prl_deployment_query', DAY_IN_SECONDS );
	...
	public function add( $args ) {
		...
		$deployment = new WC_PRL_Deployment_Data(
			array(
				'active'          => $args['active'],
	...
	public function clear_caches( $ids ) {
		...
		// Invalidate cache for each deployment meta.
		foreach ( $ids as $id ) {
			WC_Cache_Helper::invalidate_cache_group( 'deploymentmeta_' . $id );
		}
