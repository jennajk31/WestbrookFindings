...
class WC_PRL_Amplifiers {
	...
	public function get_admin_filters_html( $engine, $options = array() ) {
		...
		?>
		<div id="os-container" class="widefat wc-prl-amplifiers-container"
			data-os_count="<?php echo (int) $amplifiers_count; ?>">
			<div class="os_boarding<?php echo $amplifiers_count ? '' : ' active'; ?>">
				<div class="icon">
					<i class="prl-icon prl-amps"></i>
				</div>
				<div
					class="text"><?php esc_html_e( 'No amplifiers found. Add one now?', 'woocommerce-product-recommendations' ); ?></div>
			</div>
			<div id="os-list"<?php echo $amplifiers_count ? '' : ' class="hidden"'; ?>>
				<?php
							...
							if ( array_key_exists( $amplifier_id, $amplifiers ) ) {
								...
								<div class="os_row" data-os_index="<?php echo esc_attr( $amplifier_index ); ?>">
									...
									<div class="os_remove column-wc_actions">
										<a href="#" role="button"
											aria-label="<?php echo esc_attr__( 'Remove amplifier', 'woocommerce-product-recommendations' ); ?>"
											data-tip="<?php echo esc_attr__( 'Remove', 'woocommerce-product-recommendations' ); ?>"
											class="button wc-action-button trash help_tip"></a>
									</div>
								</div>
								<?php
							}
	...
	private function get_amplifiers_dropdown( $engine_type, $amplifiers, $selected_id, $additional_options = array() ) {
		...
		<select class="os_type">
			...
			if ( ! empty( $additional_options ) ) {
				...
				foreach ( $additional_options as $key => $value ) {
					...
					<option value="<?php echo esc_attr( $key ); ?>"
							selected="selected"><?php echo esc_html( $value ); ?></option>
					<?php
				}
			}
			...
			foreach ( $amplifiers as $amplifier_id => $amplifier ) {
				?>
				<option
					value="<?php echo esc_attr( $amplifier_id ); ?>" <?php echo $amplifier_id === $selected_id ? 'selected="selected"' : ''; ?>>
					<?php
		...
		foreach ( $types as $type => $label ) {
			...
			<script type="text/template" id="tmpl-wc_prl_engine_<?php echo esc_attr( $type ); ?>_amplifier_row">
				<div class="os_row" data-os_index="{{{ data.os_index }}}">
					...
					<div class="os_remove column-wc_actions">
						<a href="#" role="button" class="button wc-action-button trash help_tip"
							aria-label="<?php echo esc_attr__( 'Remove amplifier', 'woocommerce-product-recommendations' ); ?>"
							data-tip="<?php echo esc_attr__( 'Remove', 'woocommerce-product-recommendations' ); ?>"></a>
					</div>
				</div>
			</script>
			...
			foreach ( $amplifiers as $amp_id => $amplifier ) {
				...
				<script type="text/template"
						id="tmpl-wc_prl_engine_<?php echo esc_attr( $type ); ?>_amplifier_<?php echo esc_attr( $amp_id ); ?>_content">
					<?php
	...
	/**
	 * Get calibration option per type.
	 *
	 * @since 4.1.0
	 *
	 * @param string $key
	 * @param string $amplifier_code
	 * @param string $calibration_type
	 *
	 * @return int|string|false False for invalid params.
	 */
	public function get_calibration_option_per_type( $key, $amplifier_code, $calibration_type ) {
		$fgt_options_low = array(
			'wc_prl_amp_fgt_time_weeks'                 => 0,
			'wc_prl_amp_fgt_foreground_order_count_min' => 5,
			'wc_prl_amp_fgt_foreground_support_pct_min' => 1,
			'wc_prl_amp_fgt_foreground_count_min'       => 3,
			'wc_prl_amp_fgt_background_count_min'       => 5,
			'wc_prl_fgt_significance_score'             => 'jlh',
			'wc_prl_amp_fgt_jlh_threshold'              => 0,
			'wc_prl_amp_fgt_gnd_threshold'              => 0.8,
		);

		$fgt_options_medium = array(
			'wc_prl_amp_fgt_time_weeks'                 => 0,
			'wc_prl_amp_fgt_foreground_order_count_min' => 10,
			'wc_prl_amp_fgt_foreground_support_pct_min' => 2,
			'wc_prl_amp_fgt_foreground_count_min'       => 5,
			'wc_prl_amp_fgt_background_count_min'       => 10,
			'wc_prl_fgt_significance_score'             => 'jlh',
			'wc_prl_amp_fgt_jlh_threshold'              => 0.1,
			'wc_prl_amp_fgt_gnd_threshold'              => 0.8,
		);

		$fgt_options_high = array(
			'wc_prl_amp_fgt_time_weeks'                 => 0,
			'wc_prl_amp_fgt_foreground_order_count_min' => 20,
			'wc_prl_amp_fgt_foreground_support_pct_min' => 4,
			'wc_prl_amp_fgt_foreground_count_min'       => 10,
			'wc_prl_amp_fgt_background_count_min'       => 20,
			'wc_prl_fgt_significance_score'             => 'jlh',
			'wc_prl_amp_fgt_jlh_threshold'              => 0.2,
			'wc_prl_amp_fgt_gnd_threshold'              => 0.8,
		);
		...
		if ( 'fgt' === $amplifier_code ) {
			switch ( $calibration_type ) {
				case 'low':
					return $fgt_options_low[ $key ] ?? false;
				case 'medium':
					return $fgt_options_medium[ $key ] ?? false;
				case 'high':
					return $fgt_options_high[ $key ] ?? false;
				default:
					return false;
			}
		}

		$oab_options_low = array(
			'wc_prl_amp_oab_time_weeks'                    => 0,
			'wc_prl_amp_oab_foreground_customer_count_min' => 5,
			'wc_prl_amp_oab_foreground_support_pct_min'    => 1,
			'wc_prl_amp_oab_foreground_count_min'          => 3,
			'wc_prl_amp_oab_background_count_min'          => 5,
			'wc_prl_oab_significance_score'                => 'jlh',
			'wc_prl_amp_oab_jlh_threshold'                 => 0,
			'wc_prl_amp_oab_gnd_threshold'                 => 0.8,
		);

		$oab_options_medium = array(
			'wc_prl_amp_oab_time_weeks'                    => 0,
			'wc_prl_amp_oab_foreground_customer_count_min' => 10,
			'wc_prl_amp_oab_foreground_support_pct_min'    => 2,
			'wc_prl_amp_oab_foreground_count_min'          => 5,
			'wc_prl_amp_oab_background_count_min'          => 10,
			'wc_prl_oab_significance_score'                => 'jlh',
			'wc_prl_amp_oab_jlh_threshold'                 => 0.1,
			'wc_prl_amp_oab_gnd_threshold'                 => 0.8,
		);

		$oab_options_high = array(
			'wc_prl_amp_oab_time_weeks'                    => 0,
			'wc_prl_amp_oab_foreground_customer_count_min' => 20,
			'wc_prl_amp_oab_foreground_support_pct_min'    => 4,
			'wc_prl_amp_oab_foreground_count_min'          => 10,
			'wc_prl_amp_oab_background_count_min'          => 20,
			'wc_prl_oab_significance_score'                => 'jlh',
			'wc_prl_amp_oab_jlh_threshold'                 => 0.2,
			'wc_prl_amp_oab_gnd_threshold'                 => 0.8,
		);

		if ( 'oab' === $amplifier_code ) {
			switch ( $calibration_type ) {
				case 'low':
					return $oab_options_low[ $key ] ?? false;
				case 'medium':
					return $oab_options_medium[ $key ] ?? false;
				case 'high':
					return $oab_options_high[ $key ] ?? false;
				default:
					return false;
			}
		}
	 ...
	 *
	 * @since 1.4.12
	 * @since 4.1.0  Added the type string for the deployment parameter (used as a callibration type).
	 *
	 * @param string                   $key            The calibraption option name.
	 * @param string                   $amplifier_code The amplifier code (fgt or oab).
	 * @param WC_PRL_Deployment|string $deployment     (Optional) A deployment object or a preset name.
	 *
	 * @return mixed
	 */
	public function get_calibration_option( $key, $amplifier_code, $calibration_type = null ) {

		$value = '';
		foreach ( $this->get_calibration_settings( $amplifier_code ) as $setting ) {
			if ( $key === $setting['filter'] ) {
				$option_value = get_option( $setting['id'], $setting['default'] );

				if ( $calibration_type instanceof WC_PRL_Deployment ) {
					// Assumes 'default' as the calibration type.
					$value = apply_filters( $setting['filter'], $option_value, $calibration_type );
				} else {
					$value = $this->get_calibration_option_per_type( $setting['id'], $amplifier_code, $calibration_type );
					$value = false === $value ? $option_value : $value;
				}
