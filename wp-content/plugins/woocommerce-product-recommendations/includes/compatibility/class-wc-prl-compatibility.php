...
class WC_PRL_Compatibility {
	...
	public static function declare_blocks_compatibility() {
		...
		\Automattic\WooCommerce\Utilities\FeaturesUtil::declare_compatibility( 'cart_checkout_blocks', WC_PRL()->get_plugin_basename(), true );
	}
	...
	public static function add_compatibility_notices() {
		...
		if ( function_exists( 'WC_PB' ) ) {
			...
			if ( version_compare( WC_PRL()->get_plugin_version( true, WC_PB()->version ), $required_version ) < 0 ) {
				...
				WC_PRL_Admin_Notices::add_dismissible_notice(
					$notice,
					array(
