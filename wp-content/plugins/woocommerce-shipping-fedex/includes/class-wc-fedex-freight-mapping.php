...
class WC_Fedex_Freight_Mapping {
	...
	public function get_shipping_classes( $classes ) {
		foreach ( $classes as $class ) {
			$class->fedex_freight_class         = version_compare( WC_VERSION, '3.6', 'ge' ) ? get_term_meta( $class->term_id, 'fedex_freight_class', true ) : get_woocommerce_term_meta( $class->term_id, 'fedex_freight_class', true );
