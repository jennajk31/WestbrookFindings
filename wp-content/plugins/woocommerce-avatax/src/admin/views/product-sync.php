...
if ( $syncing ) {
	$button_label = __( 'Stop Sync', 'woocommerce-avatax' );
	$toggle_label = __( 'Sync', 'woocommerce-avatax' );
} else {
	$button_label = __( 'Sync', 'woocommerce-avatax' );
	$toggle_label = __( 'Stop Sync', 'woocommerce-avatax' );
}
...
<tr style="vertical-align: top;">
	...
	<td class="forminp forminp-product-sync" data-address-id="<?php echo esc_attr( $id ); ?>">
		...
		<p class="wc-avatax-product-sync validate">
			<span class="connection-failed" style="display: none;"><?php esc_html_e( 'Check your credentials and ensure you have the subscription to continue. You can also check your subscription details with your customer account manager.', 'woocommerce-avatax' ); ?></span>
			...
			<p class="description">Select Sync to sync your product catalog in WooCommerce with Avalara.</p>
			<br/><span class="syncing" style="<?php echo $syncing ? '' : 'display: none;'; ?>"><?php esc_html_e( 'Your catalog is syncing to Avalara.', 'woocommerce-avatax' ); ?></span>			
