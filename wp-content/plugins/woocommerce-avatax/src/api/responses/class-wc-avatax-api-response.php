...
class WC_AvaTax_API_Response extends Framework\SV_WC_API_JSON_Response {
	...
	public function get_errors() {
		...
		if ( ! empty( $this->response_data->error ) ) {
			...
			// Add check for details property
			if ( ! empty( $error->details ) && ( is_array( $error->details ) || is_object( $error->details ) ) ) {
		...
		if ( ! empty( $this->response_data->messages ) && ( is_array( $this->response_data->messages ) || is_object( $this->response_data->messages ) ) ) {
	 ...
	 * @since 2.8.1
	 *
	 * @param float $response_time time required to perform the API call
	 * @return void
	 */
	public function set_response_time($response_time) {
	 ...
	 * @since 2.8.1
	 *
	 * @return float response time required to perform the API call
	 */
