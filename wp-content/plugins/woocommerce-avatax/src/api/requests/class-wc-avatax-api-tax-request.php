...
class WC_AvaTax_API_Tax_Request extends \WC_AvaTax_API_Request {
	...
	public function process_order( WC_Order $order, $record_transaction ) {
		...
		if ( $record_transaction && $order->get_meta( '_wc_avatax_origin_address' ) ) {
		...
		// Added condition to check origin_address available and not null
		if (is_null($origin_address) || $origin_address === false) {
			$origin_address = [];
		}
	...
	protected function prepare_address_data( $origin, $destination, $billingAddress = null ) {
		...
		if(!empty($billingAddress) && !empty($billingAddress["city"]) && !empty($billingAddress["state"]) && !empty($billingAddress["country"]) && !empty($billingAddress["postcode"]) ){
			$addresses['pointOfOrderOrigin'] =  $this->prepare_address( $billingAddress );
		}
	...
	protected function remove_duplicate_addresses( $data ) {
		...
		$types = array( 'shipFrom', 'shipTo', 'pointOfOrderOrigin' );
