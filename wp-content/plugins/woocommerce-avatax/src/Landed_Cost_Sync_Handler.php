...
class Landed_Cost_Sync_Handler {
	...
	protected function prepare_hs_item_model( WC_Product $wc_product ) : HS_Item_Model {
		...
		// tax code stored in AvaTax Item Main, from where it may be synced back to product meta during a `sync` action.
	...
	protected function get_remote_item( WC_Product $wc_product ) {
		...
		try {
			...
			//Logging error
			wc_avatax()->logger()->log_exception("ItemSync", "get_remote_item", $exception->getMessage(), $exception->getTraceAsString());
		}
	 ...
	 *
	 * Stores classification IDs for existing classifications in AvaTax Item Main for the given product and returns
