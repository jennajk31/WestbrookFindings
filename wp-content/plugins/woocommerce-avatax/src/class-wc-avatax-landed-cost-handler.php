...
class WC_AvaTax_Landed_Cost_Handler {
	...
	public function can_connect_to_hs_api() : bool {
		...
		try {
			...
			//Logging error
			wc_avatax()->logger()->log_exception("ItemSync", "can_connect_to_hs_api", $exception->getMessage(), $exceptione->getTraceAsString());
	...
	/**
	 * Gets the supported states list for selected country.
	 *
	 * @since 2.7.0
	 * @param string country
	 * @return string[] array of state codes for selected country
	 */
	public function get_supported_states(string $country) : array {
		$this->selected_country =  $country;
		return (array) $this->safely_get_api_result( function( WC_AvaTax_API $api,string $country) {
			return $api->get_nexus_list()->get_state_list($country);
		}, 'wc_avatax_landed_cost_supported_states_' . $country );
