        ...
        "jsontree": function(command) {
            ...
            function commandMode(option) {
                var commands = {
                    ...
                    "getSelectedItemPaths" : getSelectedItemPaths,
                    "selectAll": selectAll,
            ...
            function initialMode() {
                ...
                jsonRoot.off('click.jsontree').on("click.jsontree", ".tree-item", function(ev) {
                ...
                if(options.selected_nodes != undefined){
                    populateSelected();
                }

                if(options.expand_default){
                    expandAll();
                }
            }
            ...
            function generateJsonTree(jsonObj, domObj, pathFlag) {
                for (var key in jsonObj) {
                    ...
                    if (valueType === "object" && !$.isEmptyObject(value)) {
                        ...
                        span2.setAttribute("class", "tree-label-separator");

                        span2.appendChild(document.createTextNode(":"));
						
						var span3 = document.createElement("span");
                        span3.setAttribute("class", "tree-label-value");

                        span3.appendChild(document.createTextNode(value));
						if(options.show_checkbox) {
                            span.appendChild(icon);
                        }
                        span.appendChild(span1);
            ...
            function getSelectedItemPaths() {
                ...
                $.each(selected, function(index, element) {
                    var $ele = $(element);
                    results.push( $ele.data("path"));
                });
                return results;
            }

            function selectAll() {
                var nodes = jsonRoot.find('.tree-item:visible');
                $.each(nodes, function(index, element) {
                    var $ele = $(element);
                    if(!$ele.hasClass("tree-selected")){
                        toggerTreeNode(element);
                    }
                });
            }

            function unSelectAll() {
                var nodes = jsonRoot.find('.tree-selected:visible');
                $.each(nodes, function(index, element) {
                    var $ele = $(element);
                    toggerTreeNode(element);
                });
            ...
            function populateSelected() {
                var elements = options.selected_nodes;
                var nodes = jsonRoot.find('.tree-item');
                $.each(nodes, function(index, element) {
                    var $ele = $(element);
                    const arrPath = $ele.data("path").split(".");
                    const shortPath = arrPath[0]+'.'+arrPath[(arrPath.length-2)]+'.'+arrPath[(arrPath.length-1)];
                    if(elements.indexOf(shortPath) !== -1){
                    toggerTreeNode(element);
                    }
                });
            }
            ...
            function expandAll() {
                $(".tree-branch-header").each(function(){
                    openFolder(this);
                });
            }
