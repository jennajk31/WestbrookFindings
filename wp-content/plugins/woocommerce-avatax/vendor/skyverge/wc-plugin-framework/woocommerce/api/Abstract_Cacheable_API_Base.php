...
abstract class Abstract_Cacheable_API_Base extends SV_WC_API_Base
{
	...
	protected function do_remote_request( $request_uri, $request_args ) {
		...
		$customerCode = (json_decode($request_args['body']) != null && property_exists(json_decode($request_args['body']), 'customerCode')) ? json_decode($request_args['body'])->customerCode : "";
		if(str_contains($request_uri, 'transactions/create') &&  get_transient("wc_avatax_api_" . $customerCode) == true)
		{
	...
	protected function handle_response( $response , bool $isRequiredAddnlParams = false ) {
		...
		parent::handle_response( $response , $isRequiredAddnlParams);
