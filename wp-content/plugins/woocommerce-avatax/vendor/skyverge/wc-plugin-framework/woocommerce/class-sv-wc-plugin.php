...
abstract class SV_WC_Plugin {
	...
	public function add_api_request_logging() {
		...
		if ( ! has_action( 'wc_' . $this->get_id() . '_elr_api_request_performed' ) ) {
			add_action( 'wc_' . $this->get_id() . '_elr_api_request_performed', array( $this, 'log_elr_api_request' ), 10, 2 );
		}
	}
	...
	public function log_api_request( $request, $response, $log_id = null ) {
		...
		if(str_contains($request["uri"], 'ceplogger')) {
			return;
		}
	...
	/**
	 * Log ELR API requests/responses
	 *
	 * @since 2.2.0
	 * @param array $request request data, see SV_WC_API_Base::broadcast_request() for format
	 * @param array $response response data
	 * @param string|null $log_id log to write data to
	 */
	public function log_elr_api_request( $request, $response, $log_id = null ) {

		$this->log_elr( "Request\n" . $this->get_api_log_message( $request, false ), $log_id );

		if ( ! empty( $response ) ) {
			$this->log_elr( "Response\n" . $this->get_api_log_message( $response, false ), $log_id );
		}
	}
	 ...
	 * @return string
	 */
	public function get_api_log_message( $data, $send_body = true ) {

		$messages = array();
		$type = isset( $data['uri'] ) && $data['uri'] ? 'Request' : 'Response';
		$messages[] = $type;
		...
		foreach ( (array) $data as $key => $value ) {
			...
			if ( 'body' === $key ) {
				$value = $send_body ? $value : ($type . " body is not included in log for data security.");
			}
	...
	/**
	 * Saves errors or messages to WooCommerce Log (woocommerce/logs/elr.txt)
	 *
	 * @since 2.0.0
	 * @param string $message error or message to save to log
	 * @param string $log_id optional log id to segment the files by, defaults to plugin id
	 */
	public function log_elr( $message ) {
		...
		$this->logger->add( 'avatax-elr', $message );
	}
